---
name: kipris
description: 한국특허청 KIPRIS Plus Open API로 특허·실용신안·디자인·상표·심판·등록사항·해외특허를 검색합니다 (49개 서비스, 540개 오퍼레이션). 사용자가 특허나 상표를 찾아달라고 하거나, 출원번호·출원인·등록권자로 조회하거나, 선행기술·경쟁사 IP 동향을 조사하거나, KIPRIS·특허검색·상표검색·patent search·prior art를 언급하면 코딩 작업 중이 아니더라도 반드시 이 스킬을 사용하세요. "AI 관련 특허 찾아줘", "삼성전자가 출원한 특허", "출원번호 1020200123456 서지정보", "이 이름으로 상표 등록됐는지 확인", "artificial intelligence 미국 특허" 같은 요청이 모두 해당합니다.
---

# KIPRIS 한국 지식재산 검색

KIPRIS Plus Open API를 호출해 한국·해외 지식재산 정보를 검색한다. 이 스킬은 **검색 도구**이지 개발 도구가 아니다 — 사용자가 코드를 작성하는 중이 아니어도 특허·상표 조회 요청이면 바로 사용한다.
